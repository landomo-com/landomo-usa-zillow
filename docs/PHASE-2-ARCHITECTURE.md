...
```bash
# Create database
createdb scraper_brazil_quintoandar

# Run schema
psql -U landomo -d scraper_brazil_quintoandar -f database/schema.sql
```
...
SCRAPER_DB_NAME=scraper_brazil_quintoandar
...
services:
  postgres:
    ...
    environment:
      - POSTGRES_DB=scraper_brazil_quintoandar
